...
def compute_eval_metrics_wala(df_test: pd.DataFrame, config_name, output_folder, data, sample=False):
    """
    ...
    for i, p in enumerate(test_programs_list):
        data_point: Dict[str, Dict[str, float]] = {}
        # try:
        program_df = df_test[df_test['program_name'] == p]
        if program_df.empty:
            data_point[config_name] = None
            data[p] = data_point
            continue
        ...
        program_df.reset_index(inplace=True)
        w_prec, w_rec, w_f1 = compute_recall_precision_f1(program_df)
        print(f"Program {p}_config {config_name} | Wala prec: {w_prec:.2f} Wala rec: {w_rec:.2f} Wala f1: {w_f1:.2f}")
        w_prec_l.append(w_prec)
        w_rec_l.append(w_rec)
        w_f1_l.append(w_f1)
        data_point[config_name] = {'P': w_prec, 'R': w_rec}
        data[p] = data_point
    ...
    if not sample:
        with open(os.path.join(output_folder,'statistics', f'config_{config_name}.json'), 'a') as file:
            json.dump({'no. programs':len(w_prec_l) ,'P': statistics.mean(w_prec_l), 'R': statistics.mean(w_rec_l), 'F1': statistics.mean(w_f1_l)}, file)
    print(f"Wala config {config_name} prec: {statistics.mean(w_prec_l):.2f} Wala rec: {statistics.mean(w_rec_l):.2f} Wala f1: {statistics.mean(w_f1_l):.2f}")
    return data
