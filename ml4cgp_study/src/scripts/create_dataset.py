...
HARD_DRIVE_PATH = '/20TB/mohammad/cg_dataset'
PROJECT_DATA_PATH = f'{HARD_DRIVE_PATH}/ml4cgp_study_data'
PROJECT_STATIC_CGS_PATH = f'{HARD_DRIVE_PATH}/static_cg'
...
def load_xcorpus_data(static_dataset_folder, dynamic_dataset_folder, use_1cfa=False):
    projects_xcorpus = [(p, 'qualitas_corpus_20130901') for p in listdir(join(static_dataset_folder, 'qualitas_corpus_20130901'))] + \
                       [(p, 'xcorpus-extension-20170313') for p in listdir(join(static_dataset_folder, 'xcorpus-extension-20170313'))]
    project_static_cg: Dict[str, Dict[str, pd.DataFrame]] = {}
    ...
    for project_name, dataset in tqdm(projects_xcorpus):
        ...
        project_static_cg[project_name] = {}
        
        dynamic_project_folder = join(dynamic_dataset_folder, dataset, project_name)
        static_project_folder = join(static_dataset_folder, dataset, project_name)
        if isdir(dynamic_project_folder):
            for file_name in listdir(dynamic_project_folder):
                if file_name == "_wiretap":
                    try:
                        dyn_cg_bt = pd.read_csv(join(dynamic_project_folder, file_name, "merged_builtin-tests_dyn_cg.csv"),
                                                names=['source', 'target'])
                        dyn_cg_gt = pd.read_csv(join(dynamic_project_folder, file_name, "merged_generated-tests_dyn_cg.csv"),
                                                names=['source', 'target'])
                        ...
                        combined_dyn_cg_path = join(dynamic_project_folder, file_name, "merged_combined-tests_dyn_cg.csv")
                        ...
                        pass
        if isdir(static_project_folder):
            for file_name in listdir(static_project_folder):
                csv_file_path = join(static_project_folder, file_name)
                config = int(file_name.split('_')[-1].replace('.csv', ''))
                if config != 0:   #TODO:only examin the default configuration
                    continue
                try:
                    project_static_cg[project_name][config] = pd.read_csv(csv_file_path)
...
def process_p(p):
        ...
        def process_row_dynamic(r):
            ...
            if "Test" in utils.extract_class_name(r.source) or "Test" in utils.extract_class_name(r.target) or \
                r.source.startswith(prefixes_to_remove) or r.target.startswith(prefixes_to_remove) or \
                r.source.startswith('org/evosuite') or r.target.startswith('org/evosuite'):
                return None
        ...
        # static
        for configID in project_static_cg[p].keys():
            m_idx_bt_st = {f"{r.source}|{r.target}": i for i, r in project_static_cg[p][configID].iterrows()}

            r_static_processed = set()
            r_static_nodes_visited = set()

            dataset_rows_static = project_static_cg[p][configID].apply(process_row_static, axis=1).dropna().tolist()
            ...
            dfs[configID] =  pd.DataFrame(dataset_rows,
...
def create_dataset(output_folder, dataset_version, project_static_cg, project_dynamic_cg):
    ...
    for p, d in static_dyn_common_pruned_df.items():
        for config, v in d.items():
            ...
            print(f"Project {p}_config_{config} has {p_no_st_edges:,} static edges and has {p_no_dyn_edges:,} dynamic edges")

    for p, d in static_dyn_common_pruned_df.items():
        for config in d:
            static_dyn_common_pruned_df[p][config].to_csv(join(output_folder, p.replace("/", "_")+f"_config_{config}_dataset_{dataset_version}.csv"), index=False)
...
def concat_datasets(output_folder, dataset_name, static_dyn_common_pruned_df):
    ...
    config_dataframes = defaultdict(list)

    for program_name, config_dict in static_dyn_common_pruned_df.items():
        for config_name, df in config_dict.items():
            config_dataframes[config_name].append(df.assign(program_name=program_name))

    data: Dict[str, Dict[str, Dict[str, float]]] = {}

    for config_name, df_list in config_dataframes.items():
        # Concatenate all DataFrames for this config
        high_tc_data_df = pd.concat(df_list, ignore_index=True)

        high_tc_data_df.to_csv(join(output_folder, f"{dataset_name}_{config_name}.csv"), index=False)

        # utils.report_training_samples_types(high_tc_data_df)
        data = eval.compute_eval_metrics_wala(high_tc_data_df, config_name, output_folder, data, False)
        ...
        # eval.compute_eval_metrics_wala(high_tc_data_df_sampled, config_name, output_folder, True)
        ...
        # high_tc_data_df_sampled.to_csv(join(output_folder, f'{dataset_name}_{config_name}_sampled.csv'), index=False)

    df = pd.DataFrame(index=static_dyn_common_pruned_df.keys(), columns=config_dataframes.keys())

    # Fill in the DataFrame with P and R values or 'X' if no config exists for that program
    for program, config_data in data.items():
        for config, values in config_data.items():
            if values:
                df.loc[program, config] = f"P: {values['P']}, R: {values['R']}"
            else:
                df.loc[program, config] = 'X'

    # Optionally, save the table as a CSV
    df.to_csv(join(output_folder,'statistics','program_config_table.csv'))
...
def read_njr1():
    path = f"{PROJECT_DATA_PATH}/njr1/cgs/njr1_programs_2.txt"
...
def load_njr1_data(programs: list, cgs_path: str, wiretap_path: str):
    ...
    for p in programs:
        ...
        s_cg_directory = join(cgs_path, p)
        # if exists(p_cg_file):
        ...
        def load_static_cg(p, s_cg_directory):
            '''read the folders in the cg folder and read each '''
            static_cgs_p = {}
            for configID in range(1,10):
                s_cg_path  = join(s_cg_directory, f"{p}_config_{configID}.csv")
                s_cg_df = pd.read_csv(s_cg_path)
                static_cgs_p[configID] = s_cg_df
            return static_cgs_p
        ...
        static_cgs[p] = load_static_cg(p, s_cg_directory)
...
if __name__ == "__main__":
    ...
    if dataset_name == 'ycorpus':
        ...
        STATIC_DATASET_FOLDER = join(PROJECT_STATIC_CGS_PATH, "xcorpus")
        DYNAMIC_DATASET_FOLDER = join(PROJECT_DATA_PATH, 'xcorpus/wiretap_xcorpus_v4')
        OUTPUT_FOLDER = join(HARD_DRIVE_PATH, "output/xcorpus")
        ...
        project_static_cg, project_dynamic_cg = load_xcorpus_data(STATIC_DATASET_FOLDER, DYNAMIC_DATASET_FOLDER, use_1cfa=True)


        static_dyn_common, _ = find_common_projects(project_static_cg, project_dynamic_cg)
        static_dyn_common_pruned_df = create_dataset(OUTPUT_FOLDER, DATASET_VERSION, project_static_cg, project_dynamic_cg)
        concat_dataset = concat_datasets(join(OUTPUT_FOLDER,'concats'), f"{xcorpus_dataset_name}_{DATASET_VERSION}", static_dyn_common_pruned_df)
        # if FEATURE_SET is not FeatureTypes.SIG_FEAT:
        ...
        CGs = join(PROJECT_STATIC_CGS_PATH, "njr1")
        DATASET_VERSION = "v1.0"
        OUTPUT_FOLDER =  join(HARD_DRIVE_PATH, "output/njr1")
        njr1_dataset_name = "njr1_dataset"

        # train_njr1_programs, valid_njr1_programs, test_njr1_programs = read_njr1_programs()
        # njr1_programs = train_njr1_programs+valid_njr1_programs+test_njr1_programs
        njr1_programs = read_njr1()

        project_static_cg, project_dynamic_cg = load_njr1_data(njr1_programs,
                                                                CGs, NJR1_FOLDER)
