...
public class PDFCallGraph {
  ...
  public static void run(String appJar, String exclusionFile)
    ...
    try {
      buildPrunedCallGraph(appJar, new FileProvider().getFile(exclusionFile));
  ...
  //   for (CGNode node : cg) {
  //       String methodName = node.getMethod().getSignature();
  //       System.out.println("\nMethod: " + methodName);
  //       System.out.println("------------------------");

  //       Iterator<CGNode> successors = cg.getSuccNodes(node);
  //       while (successors.hasNext()) {
  //           CGNode target = successors.next();
  //           System.out.println("  Calls: " + target.getMethod().getSignature());
  //       }
  //   }
  // }
