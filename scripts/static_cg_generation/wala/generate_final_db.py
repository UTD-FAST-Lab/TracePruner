...
output_dir = "/20TB/mohammad/xcorpus-total-recall/static_cgs/wala/final"
input_dir = "/20TB/mohammad/xcorpus-total-recall/static_cgs/wala"

for version in os.listdir(input_dir):
    if version.startswith("v"):
        print(f"Processing version: {version}")
        version_dir = os.path.join(input_dir, version)
        for program in os.listdir(version_dir):
            program_dir = os.path.join(version_dir, program)
            for scg_file in os.listdir(program_dir):
                config_id = scg_file.split('_')[1].split('.')[0]

                output_name = f"wala_{version}_{config_id}.csv"
                file_output_dir = os.path.join(output_dir, program)
                file_output_path = os.path.join(file_output_dir, output_name)
                if not os.path.exists(file_output_path):
                    input_file_path = os.path.join(program_dir, scg_file)
