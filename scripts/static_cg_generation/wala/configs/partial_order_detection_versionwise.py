...
static_cgs_dir = '/20TB/mohammad/xcorpus-total-recall/static_cgs/wala'
config_csv = "/home/mohammad/projects/TracePruner/scripts/static_cg_generation/wala/configs/wala_1change_configs_v2.csv"
config_file_version = "v2"
...
def find_diff(config1_id, config2_id):
    ...
    scgs_dir = os.path.join(static_cgs_dir, config_file_version)

    for program in os.listdir(scgs_dir):

        scg1_path = os.path.join(scgs_dir, program, f"wala_{config1_id}.csv")
        if not os.path.exists(scg1_path):
            print(f"Static call graph for config {config1_id} not found in {program}. Skipping.")
            continue
        scg2_path = os.path.join(scgs_dir, program, f"wala_{config2_id}.csv")
...
def write_diff_to_csv(program, config1_id, config2_id, diff):
    ...
    """
    diff_dir = os.path.join(output_dir, 'wala', config_file_version, program)
