...
object DoopPCExtractorTest {
  ...
  private def computeCallSite(
        ...
        if (calls.size <= number && method.isBridge) {
            computeCallSite(classFile, p, declaredTgt, number, callerMethod, resolveBridgeMethod(classFile, p, method), outputFile)
        } else {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val outputFile = new File(args(7))
    

    // val project = Project(inputJar, jdkPath)
    // val project = Project(jdkPath)
    ...
    project.classFile(ObjectType(callerClass)) match {
      case Some(cf) =>
        ...
        cf.findMethod(callerMethod, md) match {
          case Some(method) if method.body.isDefined =>
            println(s"found method: $callerMethod in class: $callerClass with signature: $callerSignature")
            println(s"method body: ${method.body.get}")

            computeCallSite(cf, project, declaredTgt, number, callerMethod, method, outputFile)
          ...
          case Some(method) =>
            println(s"method body not found for ${method.body}")
            return
        }
      ...
      case None =>
        println(s"Class file for $callerClass not found in the project.")
        return
