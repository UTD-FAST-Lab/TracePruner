...
from transformers import AutoTokenizer, T5EncoderModel, RobertaTokenizer
...
model = T5EncoderModel.from_pretrained("Salesforce/codet5p-770m").to(device)
model.eval()
...
OUTPUT_DIR = "/20TB/mohammad/xcorpus-total-recall/features/semantic/codet5/raw"
...
for program in program_list:
    ...
    for scg_file in os.listdir(program_dir):
        ...
        for i in tqdm(range(len(df))):
            ...
            with torch.no_grad():
                embedding = model(ids_tensor, attention_mask=mask_tensor, return_dict=False)[0]
        ...
        # Save to CSV
        columns = ["method", "offset", "target"] + [f"emb_{i}" for i in range(len(embedding_np))]
