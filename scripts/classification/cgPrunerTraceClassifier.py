...
class CGPTraceClassifier:
    def __init__(self, feature_file, cgpruner_file, results_dir):
        self.feature_file = feature_file
        self.cgpruner_file = cgpruner_file
        self.results_dir = results_dir
        self.model = None
        self.results_file = os.path.join(results_dir, "classification_results_cgp_mlp.txt")
        os.makedirs(self.results_dir, exist_ok=True)
    
    def load_data(self):
        """Loads and merges feature vectors with cgpruner data."""
        df_features = pd.read_csv(self.feature_file)
        df_cgpruner = pd.read_csv(self.cgpruner_file)
        
        df_cgpruner_filtered = df_cgpruner.drop(columns=["wiretap", "wala-cge-0cfa-noreflect-intf-direct", "wala-cge-0cfa-noreflect-intf-trans", "offset"], errors='ignore')
    ...
    def preprocess_data(self, X, y):
        """Normalizes data and splits into training, validation, and test sets."""
    ...
    def check_overfitting(self, X_train, X_val, y_train, y_val):
        ...
        plt.legend(loc="best")
        plt.grid()
        plt.savefig(os.path.join(self.results_dir, "learning_curve_cgp_mlp.png"), dpi=300)
        plt.close()

if __name__ == "__main__":
    dataset_dir = '/home/mohammad/projects/CallGraphPruner/data/datasets/cgs'
    ...
    results_dir = '/home/mohammad/projects/CallGraphPruner/data/results/cgs'
    
    classifier = CGPTraceClassifier(feature_file, cgpruner_file, results_dir)
