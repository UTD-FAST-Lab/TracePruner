...
false_edges_dir = f'/20TB/mohammad/xcorpus-total-recall/false_edges'
...
def get_false_edges(total_df, program):
    """Get false edges for a specific program by reading from the false edges directory"""
    
    false_path = os.path.join(false_edges_dir, program, 'false_edges.csv')
    
    if not os.path.exists(false_path):
        return pd.DataFrame(columns=total_df.columns)  # or handle it as needed

    false_df = pd.read_csv(false_path).drop_duplicates(subset=['method', 'offset', 'target'])
    ...
    return total_df[total_tuples.isin(false_tuples)].reset_index(drop=True).drop_duplicates()
...
def main():
    for tool in ['wala', 'doop', 'opal']:
        scgs_dir = f"/20TB/mohammad/xcorpus-total-recall/static_cgs/{tool}/final"
        output_dir = f'/20TB/mohammad/xcorpus-total-recall/dataset2/{tool}'
        for program in os.listdir(scgs_dir):
            program_dir = os.path.join(scgs_dir, program)
            for scg_name in os.listdir(program_dir):
                ...
                false_df = get_false_edges(total_df, program)
