...
model = BERT()
model.load_state_dict(torch.load("finetuned_codebert_test_xcorp.pth"))
model.to(device)
model.eval()

# === CONFIG ===
BENCHMARK_CALLGRAPHS = "/20TB/mohammad/xcorpus-total-recall/test"      # contains <program>/wala0cfa.csv
PROCESSED_DATA = "/20TB/mohammad/xcorpus-total-recall/features/semantic/source_code"      # contains <program>/code.csv
PROGRAM_LIST_FILE = "/20TB/mohammad/xcorpus-total-recall/features/semantic/programs.txt"    # or training list

OUTPUT_DIR = "/20TB/mohammad/xcorpus-total-recall/features/semantic/finetuned/tool_config/doop/v1_39/1"                  # where program-wise CSVs go
